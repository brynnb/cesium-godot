...
public:
	...
	static inline glm::dvec3 engine_to_ecef(const Vector3& vec) {
		// Godot is Y-up with forward along -Z. Cesium's Cartesian/ECEF frame is
		// Z-up. This is the exact inverse of ecef_to_engine below.
		return glm::dvec3(vec.x, -vec.z, vec.y);
	}
	...
	static inline Transform3D from_glm_mat4(const glm::dmat4& m) {
	    ...
	    t.basis.rows[0].y = m[1][0];
	    t.basis.rows[0].z = m[2][0];

	    t.basis.rows[1].x = m[0][1];
	    t.basis.rows[1].y = m[1][1];
	    t.basis.rows[1].z = m[2][1];

	    t.basis.rows[2].x = m[0][2];
	    t.basis.rows[2].y = m[1][2];
