...
func _update_tilesets() -> void:
	for tileset in self.tilesets:
		...
		# The runtime owns the georeference transform so selection and rendered
		# placement always use the same float64 local-horizontal frame.
		tileset.update_tileset(self.global_transform)
